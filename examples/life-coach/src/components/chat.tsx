...
export function Chat({ user, onToolLatency }: ChatProps) {
  ...
  // Track tool latencies from message parts
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col h-full border border-[var(--border)] rounded-lg">
      <div className="px-4 py-2 border-b border-[var(--border)] flex-shrink-0">
        <span className="text-sm text-[var(--text-muted)]">Chatting as</span>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 min-h-0">
        {messages.length === 0 && (
          <div className="text-center text-[var(--text-muted)] text-sm py-8">
            Share what&apos;s on your mind — goals, struggles, or just check in
          </div>
        )}
                ...
                if (part.type === "tool-recallMemory" || part.type === "tool-storeMemory") {
                  ...
                  return (
                    <span key={i} className="text-xs text-[var(--text-muted)] italic block">
                      {part.type === "tool-recallMemory" ? "🧠 recalling..." : "💾 remembering..."}
                    </span>
                  );
                }
                return null;
      ...
      <form onSubmit={handleSubmit} className="p-3 border-t border-[var(--border)] flex-shrink-0">
        <div className="flex gap-2">
          ...
          <button
            ...
            className="px-4 py-2 rounded-lg bg-[var(--accent)] text-[var(--bg)] text-sm font-medium disabled:opacity-50 hover:opacity-90 transition-opacity"
          >
            Send
          </button>
        </div>
